...
} = require("./Authorized_Endtrypoint");
//=========================================
...
try {
  ...
  redisClient.on("error", (err) =>
    console.log("error during connecting to redis server ...")
  );
  // =======================================
} catch (e) {
  return res.json({
  ...
  if (error) {
    ...
    if (isUserExist != null) {
      ...
      if (req.body.password !== req.body.confirm)
        res.json({ error: true, message: "Your password not matched" });
      if (req.body.role == "podcaster" && req.body.personal_secret == null) {
        ...
        const hassPassword = await bcryptGenerator.hash(
          req.body.password,
          salt
        );
      ...
      res.json({
        error: false,
        message:
          "You have not been registed successfully, you have 2 minutes to verify this sent code.",
      });
    }
  }
});
  ...
  if (req.body.new_password === req.body.confirm) {
    ...
    if (get_email_cache) {
      ...
      return res.json({
        error: true,
        message:
          "Please request to server again becuase your verified time is expired",
      });
    }
  } else {
    return res.json({
...
optsender.post("/desktop/logOut", desktopLogout, (req, res) => {
  ...
  const refreshWithoutUser = generateToken(
    ...
    process.env.PROGRAM_REFRESH_TOKEN_SECRET,
    31104000000
...
optsender.post("/akara/credential", async (req, res) => {
  ...
  const hassClientSecret = await bcryptGenerator.hash(
...
optsender.post("/akara/desktop/access/token", refreshMiddleware, (req, res) => {
  ...
  const accessToken = generateToken(
    ...
    process.env.PROGRAM_TOKEN_SECRET,
    "1h"
  ); // expire in 1h after generated
        ...
        try {
          ...
          if (error)
            res.status(400).json({
              error: true,
              message:
                "You have some problem with data form submission missing attribute, contact to the owner. ",
...
optsender.listen(PORT, function () {
  console.log(
    `AKARA AUTHENTICATION SERVER IS BEING LISTENNED ON PORT : ${PORT}`
  );
});
